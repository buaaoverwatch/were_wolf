...
from app_http_method import Register, Login, Logout, create_room, join_room, get_record, updata_user_info, \
    get_room_info, get_room_list
from app_db.models import UserInfo
...
def searchUser(request):
    ...
    message = {'search_num':'0','data':[]}
    try:
        users = UserInfo.objects.filter(user_name__icontains=name)
        message['search_num']=str(len(users))
        for i in range(len(users)):
            data = {}
            data['user_id'] =str(users[i].id)
            data['user_name'] =users[i].user_name
            message['data'].append(data)

    except ObjectDoesNotExist:
        message['search_num'] = '0'
...
def getFriendList(request):
    ...
    data = json.loads(key[0])
    user_id = data['user_id']
    message = {'friend_num': '0', 'data': []}
    try:
        user = UserInfo.objects.get(id=int(user_id))
        temp = user.friends
        temp = temp.split(',')
        message['friend_num']=str(len(temp)-1)
        i=0
        while(i<len(temp)-1):
            data = {}
            user2 = UserInfo.objects.get(id=int(temp[i]))
            data['id'] = temp[i]
            data['name']=user2.user_name
            if user2.user_name in glob.user_name:
                data['state']= '1'
                if glob.user_room_id[str(user2.id)] != -1:
                    data['inRoom'] = '1'
                else:
                    data['inRoom'] = '0'
            else:
